...
```{r}
raw <- read.csv("/Users/marcosmicheloni/Documents/Master/CS-6242/dva-project/top 100 billboard scrape code/Top 50 Songs.csv")
head(raw)
```
...
clean2$top_levels <- as.factor(ifelse(clean2$rank<=10, "Top 10", ifelse(clean2$rank>25, "Top 26-50","Top 11-25")))
head(clean2)
```
...
chart_list3 <- list()
for(col in chart_cols){
  g <- ggplot(clean2[-145,], aes_string(y=col, x="as.factor(year)", col="rank")) +
            geom_point(alpha=0.6) +
            ggtitle(col) +
            theme(axis.title.x = element_blank(),
                  legend.position = "top",
